...
def handle_commandline():
    ...
    parser.add_argument("-ls", action='store_true',
                        ...
                        "[default: %(default)s]")
    parser.add_argument("-npo", nargs=2, default=(40, 120), type=int,
    ...
    return inputdata
    # return args.f, args.emin, args.de, args.npts
...
if __name__ == "__main__":
    ...
    # raise TypeError('Some variables are global now, check iT!')

    tmp = readsigma(infile)
    w = tmp[0]
    f = tmp[1]
    ...
    if not use_moments:
        print('%4s %4s %4s %4s %9s %15s %16s %16s\b' % ('npo', 'nne', 'try', 'ils',
                                                        'delta', 'm0', 'm1', 'm2'))
    else:
        print('%4s %4s %4s %4s %9s' % ('npo', 'nne', 'try', 'ils', 'delta'))
        ...
        for ine in range(ne[0], ne[1]):
            for q1 in range(0, qq2, 2):
                ...
                for qq in range(0, nrandomcycle):
                    ...
                    # tmp = choose_random_points(w, f, nne, npo)
                    e1 = tmp[0]
                    f1 = tmp[1]
